...
function myFunction(xml) {
    ...
    var defs = d3.select("#heroList").append("defs");
    ...
    d3.select("#heroList").append("g").selectAll("circle")
        ...
        .on("mouseover", function(d) {
            ...
            div.html("<strong>"+ d.Name + "</strong><br> Movies Left: " + calculateMovieLeftHero(dictEdge[d.ID].length,hero_movie_left[d.ID]))
        ...
        dictMovies.forEach(function(d,i){
            ...
            svgnew.append("text").attr("x",(dim[0]-(d.Name.length*4))).attr("y","40").text(d.Name);
...
function calculateMovieLeftHero(movieTot, NumMovie){
    if(NumMovie == undefined)
        return movieTot;
    else
        return movieTot - NumMovie.length;
}
